...
public class Mario extends CollidableObject{
	 ...
	 */
	TestingFlags flags = new TestingFlags();
	...
	private static final float INITIAL_JUMP_HEIGHT = -.65f; 
	private static final float JUMP_MULTIPLIER = .65f;
	...
	private static final int ANIM_TIME = 725;
	...
	SendingThread st = null;
	MessagePassing mp = null;
	public Mario(MarioSoundManager22050Hz soundManager) {
		
		super(STARTING_X, STARTING_Y, soundManager);
		
		//writer.WriteToFile("Mario::Mario(MarioSoundManager22050Hz soundManager)");
	...
	public int getHealth() {
		//writer.WriteToFile("Mario::int getHealth()");
		return health;
	}
	
	public boolean isInvisible() {
		//writer.WriteToFile("Mario::boolean isInvisible()");
		return isInvisible;
	}
	
	public boolean isOnSlopedTile() { 
		//writer.WriteToFile("Mario::boolean isOnSlopedTile()");
		return onSlopedTile; }

	public void setIsJumping(boolean isJumping) { 
		//writer.WriteToFile("Mario::setIsJumping(boolean isJumping)");
		this.isJumping = isJumping; }

	public boolean isJumping() { 
		//writer.WriteToFile("Mario::boolean isJumping()");
		return isJumping; }
	
	private void slowSpeed(int slowFactor) { 
		//writer.WriteToFile("Mario::slowSpeed(int slowFactor)");
		setdX(getdX()/slowFactor);	}
	
	private void accelerateFall() { 
		//writer.WriteToFile("Mario::accelerateFall()");
		setdY(-getdY()/4); }
	
	public boolean isAbovePlatform() { 
		//writer.WriteToFile("Mario::boolean isAbovePlatform()");
	...
	private void platformStatus() {
		//writer.WriteToFile("Mario::platformStatus()");
	...
	public void setAnimation(Animation newAnim) {
		//writer.WriteToFile("Mario::setAnimation(Animation newAnim)");
	...
	private Platform getPlatformCollisionX(TileMap map, float oldX, float oldY, float newX, float newY) {
		...
		//writer.WriteToFile("Mario::Platform getPlatformCollisionX(TileMap map, float oldX, float oldY, float newX, float newY)");
	...
	private Platform getPlatformCollisionY(TileMap map, float oldX, float oldY, float newX, float newY) {
		...
		//writer.WriteToFile("Mario::Platform getPlatformCollisionY(TileMap map, float oldX, float oldY, float newX, float newY)");
	...
	public void update(TileMap map, float time) {
		...
		//writer.WriteToFile("Mario::update(TileMap map, float time)");
		...
		if(isAbovePlatform) {
			...
			/*TestingFlags.isAbovePlateform=true;
			//System.out.println("TestingFlags.isAbovePlateform: "+ TestingFlags.isAbovePlateform);
			try {
				Thread.sleep(50);
			...
			/*TestingFlags.isAbovePlateform=false;
			//System.out.println("TestingFlags.isAbovePlateform: "+ TestingFlags.isAbovePlateform);
			try {
				Thread.sleep(50);
			} catch (InterruptedException e) {
		...
		if(oldY > GameRenderer.tilesToPixels(map.getHeight()) - getHeight()) { // Off the bottom of the map.
			//writer.WriteToFile("Mario has died.");
		...
		if(Float.compare( getX() , previousX) == 0 && Float.compare( getY(), previousY) == 0)
		{
			//	System.out.println("( "+getX()+" + "+previousX+") - ("+getY()+" + "+previousY+" )");	
		}
		else
		{
			previousX = getX();
	...
	public void marioToTileToBaddieCollide(GameTile tile) {
		//writer.WriteToFile("Mario::marioToTileToBaddieCollide(GameTile tile)");
	...
	public void playerCollision(TileMap map, Creature creature)  {
		...
		if(creature.isCollidable() )
		{			
			Double resultX = (double) (getX() - creature.getX());
			resultX = Math.abs(resultX);
			Double resultY = (double) (getY() - creature.getY());
			resultY = Math.abs(resultY);
			if(resultX < 50 && resultY < 15 ){
					mp = new MessagePassing();
					mp.start();
			}
			if(resultX == 0 && resultY == 0)
			{
				flags.setIsCollidingWithCreature(true);
				flags.writeToFile();
			}
		}
		//writer.WriteToFile("Mario::playerCollision(TileMap map, Creature creature)");		
		...
		if(!creature.isPlatform() && creature.isCollidable()) { 
			...
			{
				/*TestingFlags.isCollidingWithCreature=false;
				//System.out.println("TestingFlags.isCollidingWithCreature: "+TestingFlags.isCollidingWithCreature);
				try {
					Thread.sleep(50);
	...
	public void getsDamaged() {
		//writer.WriteToFile("Mario::getsDamaged()");
		if(grace == 0) {
			health--;
			if(health <= 0) {
				if(health == 0) {
	...
	public void creatureHop() {
		//writer.WriteToFile("Mario::creatureHop()");
		isJumping = true;
		setY(y -5); // fix offset
		if(!isShiftHeld) {
	...
	public void toggleMovement(int type) {
		//writer.WriteToFile("Mario::toggleMovement(int type)");
	...
	public void fixJumping() {
		//writer.WriteToFile("Mario::fixJumping()");
    ...
    public void keyPressed(KeyEvent e) {
    	...
    	//writer.WriteToFile("Mario::keyPressed(KeyEvent e)");
    ...
    public void keyReleased(KeyEvent e) {
    	//writer.WriteToFile("Mario::keyReleased(KeyEvent e)");
        int key = e.getKeyCode();
        ...
        if (key == KeyEvent.VK_LEFT) {
        	...
        	flags.setIsMovingBackward(true);
        	flags.writeToFile();
			/*System.out.println("TestingFlags.isMovingBackward: "+TestingFlags.isMovingBackward);
			try {
				Thread.sleep(50);
			} catch (InterruptedException e1) {
				// TODO Auto-generated catch block
        ...
        if(key == KeyEvent.VK_RIGHT) {
        	...
        	flags.setIsMovingForward(true);
        	flags.writeToFile();
			/*System.out.println("TestingFlags.isMovingForward: "+TestingFlags.isMovingForward);
			try {
				Thread.sleep(50);
			} catch (InterruptedException e1) {
				// TODO Auto-generated catch block
        ...
        if(key == KeyEvent.VK_SPACE) {
        	...
        	flags.setIsJumping(true);
        	flags.writeToFile();
        	/*System.out.println("TestingFlags.isJumping: "+TestingFlags.isJumping);
			try {
				Thread.sleep(50);
			} catch (InterruptedException e1) {
				// TODO Auto-generated catch block
