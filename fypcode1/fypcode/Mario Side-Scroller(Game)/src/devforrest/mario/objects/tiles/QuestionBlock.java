...
public class QuestionBlock extends GameTile {
	...
	public QuestionBlock(int pixelX, int pixelY, TileMap map, MarioSoundManager22050Hz soundManager, boolean hasCoin,
		...
		//writer.WriteToFile("QuestionBlock::QuestionBlock(int pixelX, int pixelY, TileMap map, MarioSoundManager22050Hz soundManager, boolean hasCoin, boolean hasMushroom)");
	...
	public void update(int time) {
		super.update(time);
		//writer.WriteToFile("QuestionBlock::update(int time)");
		if(getOffsetY() != 0) { setOffsetY(getOffsetY() + 2); }
	}
	
	public void doAction() {
		//writer.WriteToFile("QuestionBlock::doAction()");
		if(isActive) {
			if(hasCoin) {
				setOffsetY(-10);
